...
export function TestimonyModal({ isOpen, onClose }: TestimonyModalProps) {
    ...
    const submissionMutation = useMutation({
        mutationFn: async () => {
            let profileUrl = "https://api.dicebear.com/7.x/adventurer/svg?seed=" + name;
        ...
        >
            ...
            >
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    {/* Affiliation */}
                    <div className="space-y-1">
                        <label className="text-sm font-medium text-black">Affiliation / Location</label>
                        <input
                            required
                            type="text"
                            value={affiliation}
                    ...
                    >
                        <input
                            ...
                            className="hidden"
                        />
                        <div className={`w-full h-32 rounded-xl flex flex-col items-center justify-center transition-all border-2 border-dashed ${previewUrl ? "border-black/50 bg-gray-50" : "bg-[#D0E0E6] text-[#5A7A85] border-transparent hover:border-[#5A7A85]/50"
                            }`}>
                            {previewUrl ? (
                                ...
                                <>
                                    ...
                                    <p className="text-sm font-medium">Choose a profile picture <span className="font-normal opacity-75">or drag it here.</span></p>
                                </>
                            )}
                        </div>
                    </div>
                    ...
                    {submissionMutation.error && (
                        <p className="text-sm text-red-500 text-center">
                            {(submissionMutation.error as any).response?.data?.error || "Failed to submit testimony. Please try again."}
                        </p>
                    )}
