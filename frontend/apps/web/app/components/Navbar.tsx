...
export default function Navbar() {
  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-[100] transition-all duration-500 transition-timing-function-[cubic-bezier(0.23,1,0.32,1)] ${
        scrolled ? "pt-4" : "pt-6"
      }`}
    >
      <nav
        className={`mx-auto transition-all duration-500 ease-out border shadow-lg ${
          scrolled
      ...
      >
        <div className="flex items-center justify-between">
          ...
          <button
            type="button"
            className="flex items-center gap-2 group cursor-pointer transition-transform active:scale-95 bg-transparent border-none p-0"
          ...
          >
            ...
            <span
              className={`font-black tracking-[4px] transition-all duration-500 ${
            ...
            {navItems.map((item) => (
              <li key={item.label}>
                <button
                  ...
                  className="relative group/nav py-1 text-sm font-bold uppercase tracking-widest text-[var(--text-muted)] hover:text-[var(--text-strong)] transition-all duration-300 cursor-pointer bg-transparent border-none"
                >
                  {item.label}
        ...
        <div
          className={`md:hidden overflow-hidden transition-all duration-500 ease-[cubic-bezier(0.23,1,0.32,1)] ${
            menuOpen ? "max-h-[300px] mt-4 opacity-100" : "max-h-0 opacity-0"
          }`}
        >
            ...
            {navItems.map((item, i) => (
              <li key={item.label}>
                <button
                  ...
                  className={`w-full text-left px-4 py-3 rounded-xl hover:bg-[var(--color-primary)]/10 hover:text-[var(--color-primary)] font-bold text-sm uppercase tracking-widest transition-all duration-300 cursor-pointer bg-transparent border-none ${
